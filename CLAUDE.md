...
### Какая версия libVLC — в бою `3.6.5`, поднимать её нельзя без проверки на ТВ

| Версия | Вердикт |
|---|---|
| **`3.6.5`** | ✅ **в бою** (билд 579). Проверена владельцем на всей библиотеке |
| `3.7.5` | ❌ **собирается, но не играет НИЧЕГО**: на живом ТВ приложение виснет при старте любого видео. Билд 576 отозван |
| `4.0.0-eap29` | ⚠️ билд 578 отозван по жалобе владельца. При этом в отладочном прогоне по adb (см. медиасервер §BL.5) он играл и файл, и HLS — то есть «сломан» не доказан, но и не нужен: 3.6.5 делает то же самое, а eap29 — превью |

⚠️ **Порядок при любом апгрейде плеера**: собрать → поставить на ТВ по `adb install` → убедиться, что видео идёт → и только потом публиковать в OTA-канал. Сервер про работоспособность плеера не знает ничего: тестов нет, эмулятора нет, сборка и `aapt dump badging` зелёные и на неиграющем билде.

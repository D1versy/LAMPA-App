...
### Какая версия libVLC — в бою `3.6.5`, поднимать нельзя без проверки на ТВ

| Версия | Вердикт |
|---|---|
| **`3.6.5`** | ✅ **в бою** (билд 584). Рядовой контент идёт аппаратно, 24 к/с |
| `3.7.5` | ❌ не играет вообще ничего, билд 576 отозван |
| `4.0.0-eap29` | ❌ чинит 4K HDR/DV, но ломает рядовой контент: на H.264 1918×802 декодер отдаёт `OMXNodeInstance ParamPortDefinition BadParameter` и картинки нет. Билды 578/581 отозваны |

**Чего это железо не тянет в принципе** (TCL, Realtek rtd2851a, 32-битный ARM) — не лечится кодом:
4K HEVC 10 бит HDR/Dolby Vision (MediaCodec отваливается → софт ~7-10 к/с) и AV1 (libVLC вообще
не отдаёт его в железо — в `libvlc.so` нет строки `video/av01` → всегда dav1d, ~16 к/с). Тот же
тайтл в 1080p H.264/HEVC играет аппаратно без запинки. Разбор и замеры — медиасервер §BL.5.

⚠️ **Порядок при любом апгрейде плеера**: собрать → `adb install` на телевизор → замерить кадры
на **рядовом** файле И на 4K HDR → и только потом публиковать в OTA-канал. Сборка и
`aapt dump badging` зелёные и на неиграющем билде; тестов и эмулятора нет.
...
**Тулчейн** (нужен, только если снова полезешь на 3.7.x или eap — в бою его НЕТ, там `compileSdk 34`), разбор — **`E:\Media-server\claude\06-fixes-and-gotchas.md` §BL.4**:
...
Фичи: резюме позиции (seek строго ПОСЛЕ первого `Event.Playing` — libVLC до старта молча игнорирует seek), auto-next по плейлисту, аудио/суб-дорожки, внешние субтитры (`addSlave` после старта), меню качества с сохранением позиции, D-pad: ←/→ перемотка ±10с (удержание 30с/60с); нажатия КОПЯТСЯ в `seekWantMs`, в плеер уходит одна перемотка через 450 мс, а тики `TimeChanged` игнорируются, пока она не доедет — иначе по сети (HLS «весь день одной записью» в D1versy Rec) живой `player.time` отдаёт старую позицию и уехать дальше одного шага нельзя (Media-server claude/06 §CE), OK — пауза+OSD, BACK — OSD/выход. `EndReached`/`Error` обрабатываются через `Handler.post` — вызов `stop()`/`setMedia()` из колбэка события = дедлок libVLC.

**Паритет с win 1.0.8 / mac 1.0.12** (порт 22.08.2026, к этому моменту ⏮/⏭, автоскип заставки
jut.su и пер-элементное меню качества на Android уже были):
- **Индикатор серии** `osdEpisode` под тайтлом OSD — «Серия 3 из 12 · Дальше: …»
  (`updateEpisodeIndicator()`, скрыт при `isIPTV` и плейлисте ≤1).
- **Тост `playerToast`** сверху по центру, гаснет сам через 3 с — «⏭ Заставка пропущена»
  из `skipIntroIfNeeded()`. Отдельная вью, а НЕ `playerStatus` (та центрированная, без
  автоскрытия, занята «Загрузка…»/ошибкой) и не системный `Toast` (на ТВ инороден).
- **Ручное ⏮/⏭ не теряет позицию**: `switchItem(delta)` кладёт секунду покидаемой серии в
  её `Item.startMs` (порт `PlayNeighbor`), возврат к ней в этом же сеансе продолжает с неё.
  Auto-next этим путём НЕ идёт — там серия действительно досмотрена.
- **Гард стейл-качества** в `parseState`: в плейлисте (>1 элемента) карта `quality{}` обязана
  содержать url своего элемента, иначе отбрасывается. Защита от чужого балансера, который
  положит одну карту всем сериям (наши плейлисты `qdl.js` качество для серий не шлют вовсе).
- `player_timecode=ask` портировать НЕ требуется: auto-next живёт внутри `PlayerActivity`,
  в WebView управление не возвращается, диалога Lampa между сериями не бывает by construction.

**Плашка статуса поверх видео** (`playerStatus`, паритет с win 1.0.3 и mac/iOS 1.0.6): «Загрузка…»
ставится в `playItem()` сразу после `player.play()` и снимается первым `Event.Playing`, при
`EncounteredError` показывается «Не удалось воспроизвести». ⚠️ При ошибке плеер **больше не
закрывается сам** (раньше был уезжающий тост + мгновенный выход): остаётся с плашкой, выход — BACK,
он и так идёт в `finishWithResult(false)`, прогресс возвращается в Lampa. Побочный эффект: битый
элемент плейлиста не «пролистывается» закрытием — auto-next остался только на `EndReached`.
Плашка не фокусируемая и не кликабельная — иначе на ТВ попала бы в фокус-навигацию пульта, а на
телефоне перехватила бы тап.

**Автоскрытие OSD — 4 с, и на паузе панель не прячется вовсе** (`bumpOsd()` ставит таймер только
при `player.isPlaying` — конвенция всех платформ, см. win 1.0.4 / iOS 1.0.7). Единая точка:
`showOsdProgressOnly()` тоже зовёт `bumpOsd()`, а не `postDelayed` напрямую — иначе перемотка на
паузе прятала индикатор через 4 с.
...
Из этого же репо собирается **вторая аппка — под смартфон**. Отдельного репозитория нет
...
🔴 **Разделы «D1versy Live» и «D1versy Rec» больше НЕ локаются здесь.** С qdl 2.54 их выдаёт
**сервер** по айди устройства (`Modules/QbitDownload/Perms.cs` в форке, выдача прав — админка
`/admin/d1v`), и это настоящий запрет: без права все 13 роутов `qdl/live/…` отдают 404. Телефон
просто не получает права — как и любое другое неразрешённое устройство, включая ТВ-сборку.
...
| системные бары | sticky-immersive везде | **тоже sticky-immersive везде** (см. «Фуллскрин на телефоне») |
...
### Фуллскрин на телефоне — бары прячем, а не показываем

Телефонная оболочка идёт в **sticky-immersive, как ТВ**: системного статус-бара сверху нет,
шапка Lampa рисуется во весь экран. Шторка и системный жест «назад» доступны свайпом от края
(`BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE`).

🔴 **Почему не «показывать бары, как на любом телефоне»** — так было раньше
(`applySystemBars()` звал `showSystemUI()` для `phoneBuild`), и это давало **слипшуюся шапку**.
Тема `Theme.LAMPA` ставит `android:windowFullscreen=true`, то есть на окне висит легаси-флаг
`FLAG_FULLSCREEN`, а он **перебивает** `insetsController.show()` — бары так и не показывались
(в `dumpsys window` окно всё время с `fl=… FULLSCREEN …`). Получалась худшая комбинация:
бары фактически спрятаны, но инсеты окна считались по-разному в зависимости от пути входа:

| путь | геометрия WebView | что видел пользователь |
|---|---|---|
| холодный старт, портрет | `screenY=128, height=2272` | мёртвая чёрная полоса выреза, часов всё равно нет |
| возврат из плеера / поворот | `screenY=0, height=2400` | статус-бар **поверх** шапки: гамбургер сливался с часами, колокольчик — с wifi/батареей |

Само не чинилось до перезапуска приложения, а возврат из плеера — самый ходовой путь в аппке.

Правки (обе в [MainActivity.kt](app/src/main/java/top/rootu/lampa/MainActivity.kt)):
1. `applySystemBars()` → всегда `hideSystemUI()`. Ветки по `phoneBuild` больше нет, **поведение ТВ
   не изменилось** (там и был `hideSystemUI`). Зовётся из `onResume()` и `onConfigurationChanged()`,
   поэтому оба пути возврата закрыты и отдельный `onWindowFocusChanged` не нужен.
2. `setupActivity()` → `layoutInDisplayCutoutMode = SHORT_EDGES` (API 28+). Без этого при
   `targetSdk 28` режим выреза `DEFAULT`, и система в портрете letterbox'ит полосу выреза чёрным —
   те самые 128 px. На ТВ выреза нет, флаг там ни на что не влияет.

Замерено на эмуляторе после фикса: портрет `1080×2400 @ (0,0)`, альбом `2400×1080 @ (0,0)` —
одинаково на холодном старте, после плеера и после поворота. В альбоме заодно ушёл левый чёрный
столб (`screenX` был 128). Проверять так: `adb forward tcp:9222 localabstract:webview_devtools_remote_<pid>`
→ `curl -s localhost:9222/json` (у debug-сборки включён `setWebContentsDebuggingEnabled`).

⚠️ Плата за это, принято осознанно: часов и батареи поверх приложения больше не видно, и Android
один раз показывает свою плашку «Viewing full screen — to exit, swipe down from the top».
...
### Как устроены телефонные правки веб-UI

Меню, кнопки и экраны рисует **серверный** `qdl.js` / бандл Lampa (форк `E:\lampac`), одни и те же
для всех клиентов, поэтому «собрать другой APK» само по себе ничего не меняет. Осталась мелкая
косметика, ради которой серверный флаг заводить не за чем: `helpers/PhoneLock.kt` инжектит в свой
WebView JS-сниппет из `MainActivity.onBrowserPageFinished`. Что в нём:
1. CSS `display:none` на `.navigation-bar__item[data-action="settings"]` — «Настройки» в нижней
   панели мобильной вёрстки;
2. сид `localStorage['language']='ru'` + автоклик по «Русский», если экран выбора успел открыться;
...
🔴 Пункт про Live/Rec (CSS-лок + обёртка `Lampa.Activity.push`) **убран в qdl 2.54** — эту работу
делает сервер, и делает лучше: там запрет, а не сокрытие. Возвращать сюда не нужно.
...
**Границы честно:** оставшиеся правки — СОКРЫТИЕ, не защита (пункты «Загрузок» и кнопка 🔔
никуда не деваются на сервере). Для Live/Rec это уже не так: там настоящий серверный запрет.

🔗 **Связность с форком:** опорные точки — значения `act` пунктов меню «Загрузок»
(`jutwatch`/`watch`/`addcol`), класс кнопок карточки jut.su `.qdl-btn-focus` и метка 🔔 из
`JUT_MODE_LABEL` — всё это живёт в `qdl.js`. Переименуют их там — правка молча перестанет работать.
**При правках меню/кнопок в `qdl.js` проверять телефонную сборку.**
...
- `App.checkForUpdates` ждёт появления победителя до 20 с (`RESOLVE_WAIT_SEC`), и только потом
  ...
  🔴 Это ожидание считает СВОЙ счётчик и не трогает `count`: тот — общий бюджет «ждём сеть +
  ждём foreground», и если тратить его здесь, то при поздно появившейся сети (скажем, на 45-й
  секунде) ожидание съело бы остаток и `Updater.check()` не выполнился бы вовсе — проверка
  обновлений молча пропала бы за весь запуск процесса. Поймано аудитом, не тестами.

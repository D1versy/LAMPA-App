...
### Какая версия libVLC — в бою `4.0.0-eap29` (3.6.5 не играет 4K HDR/DV)

| Версия | Вердикт |
|---|---|
| **`4.0.0-eap29`** | ✅ **в бою** (билд 581). Единственная, на которой приставка декодирует 4K HEVC Main10 HDR/Dolby Vision железом — замерено 24 к/с по постам видеослоя |
| `3.6.5` | ⚠️ аварийный откат (билд 579 в `_archive/`): играет всё, **кроме 4K HDR/DV** — там MediaCodec отваливается за полсекунды, софтовый декодер даёт ~7 к/с и экран выглядит чёрным |
| `3.7.5` | ❌ не играет вообще ничего, билд 576 отозван |

⚠️ **Порядок при любом апгрейде плеера**: собрать → `adb install` на телевизор → проверить
воспроизведение на **4K HDR-файле** → и только потом публиковать в OTA-канал. Сборка и
`aapt dump badging` зелёные и на неиграющем билде; тестов и эмулятора нет. Рецепт отладки
(отдельный пакет `.dbg`, `-vv`, запуск плеера интентом, счётчик кадров) — медиасервер §BL.5.
